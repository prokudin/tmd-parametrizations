...
*...CHECK OF X AND Q2 VALUES : 
      IF ( (X.LT.0.99D-9) .OR. (X.GT.1.D0) ) THEN
         WRITE(6,91) 
  91     FORMAT (2X,'PARTON INTERPOLATION: X OUT OF RANGE GRV')
         STOP
      ENDIF
      IF ( (Q2.LT.0.799) .OR. (Q2.GT.1.01E6) ) THEN
         WRITE(6,92) 
  92     FORMAT (2X,'PARTON INTERPOLATION: Q2 OUT OF RANGE GRV')
         STOP
      ENDIF
...
*...INTERPOLATION :
      ...
      XV = X**0.5
      XS = X**(-0.2)
      UV = FINT3(NARG,XT,NA,ARRF,XUVF) * X1**3 * XV
      DV = FINT3(NARG,XT,NA,ARRF,XDVF) * X1**4 * XV
      DE = FINT3(NARG,XT,NA,ARRF,XDEF) * X1**7 * XV
      UD = FINT3(NARG,XT,NA,ARRF,XUDF) * X1**7 * XS
      US = 0.5 * (UD - DE)
      DS = 0.5 * (UD + DE)
      SS = FINT3(NARG,XT,NA,ARRF,XSF)  * X1**7 * XS
      GL = FINT3(NARG,XT,NA,ARRF,XGF)  * X1**5 * XS 
*
 60   RETURN
      END
*
...
*...INTERPOLATION :
      ...
      X1 = 1.- X
      XS = X**(-0.2)
      F2L = FINT3(NARG,XT,NA,ARRF,XF2LF) * X1**3 * XS
      F2C = FINT3(NARG,XT,NA,ARRF,XF2CF) * X1**7 * XS
      F2B = FINT3(NARG,XT,NA,ARRF,XF2BF) * X1**7 * XS
      F2  = F2L + F2C + F2B
*
 60   RETURN
      END
...
*
*
      FUNCTION FINT3(NARG,ARG,NENT,ENT,TABLE)
*********************************************************************
*                                                                   *
